       ...
       function getJson(newId){
          ...
          var selectChoose = $('#'+id+' option:selected').val();
           ...
           $.getJSON(projectName + "/subLoc.json?locId=" + selectChoose,function(json){
	         	if (json.subLocList == null){return;}
	         	$('#'+id).parent().after($('<a class="emng-select org"><strong class="emng-curselect"></strong><select name="" class="loc" id="dis'+newId+'" onchange="getLoc(id)"><option value=""></option></select></a>'));
                var select_root = $('#dis'+newId);
                var areaArr = json.subLocList;
                for (var i = 0; i < areaArr.length; i++) {
                   arealist+='<option value="'+areaArr[i].registerLoc+'">'+areaArr[i].locName+'</option>';
                }
